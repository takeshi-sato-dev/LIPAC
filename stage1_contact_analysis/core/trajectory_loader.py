...
def select_proteins(universe, n_proteins=None, apply_residue_offset=False):
    """Select proteins - optionally convert residue numbers
    ...
    apply_residue_offset : bool, default False
        If True, apply RESIDUE_OFFSET conversion to residue numbers
    ...
    """
    from ..config import RESIDUE_OFFSET, TM_HELIX_RESID_RANGE

    proteins = {}

    # Auto-detect proteins if n_proteins not specified
    if n_proteins is None:
        all_segids = ['PROA', 'PROB', 'PROC', 'PROD']
        segids = []
        for segid in all_segids:
            try:
                test_sel = universe.select_atoms(f"segid {segid}")
                if len(test_sel) > 0:
                    segids.append(segid)
            except:
                pass
        if len(segids) == 0:
            print("Warning: No proteins found with standard segids (PROA/PROB/PROC/PROD)")
            return proteins
    else:
        segids = ['PROA', 'PROB', 'PROC', 'PROD'][:n_proteins]

    # Determine residue range to use
    if TM_HELIX_RESID_RANGE is not None:
        resid_range = TM_HELIX_RESID_RANGE
    else:
        # Auto-detect: select all protein atoms
        resid_range = None

    for i, segid in enumerate(segids, 1):
        protein_name = f"Protein_{i}"
        # Select full length protein for contact analysis
        try:
            # Always select full protein
            orig_selection = universe.select_atoms(f"segid {segid}")

            if len(orig_selection) > 0:
                actual_resids = orig_selection.residues.resids
                min_resid = actual_resids.min()
                max_resid = actual_resids.max()
                print(f"{protein_name}: {len(orig_selection)} atoms, "
                      f"{len(orig_selection.residues)} residues")
                print(f"  Full protein residue range: {min_resid}:{max_resid}")
                if resid_range is not None:
                    print(f"  TM helix range (for dimer detection): {resid_range}")
            else:
                print(f"{protein_name}: No protein atoms found in segid {segid}")

            proteins[protein_name] = orig_selection

            # Optionally display converted residue ID range
            if apply_residue_offset and resid_range is not None:
                start, end = resid_range.split(':')
                converted_range = f"{int(start)+RESIDUE_OFFSET}:{int(end)+RESIDUE_OFFSET}"
